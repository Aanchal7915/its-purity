...
const CheckoutPage = () => {
    ...
    return (
        <div className="min-h-screen bg-purevit-secondary py-10 md:py-24 px-4 sm:px-6 lg:px-8">
            <div className="max-w-7xl mx-auto">
                <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 md:mb-16 gap-4 md:gap-8">
                    <div className="space-y-4">
                        ...
                        <h1 className="text-3xl md:text-6xl font-serif text-purevit-dark leading-none">Order <br /><span className="italic text-purevit-primary">Completion.</span></h1>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 md:gap-12">
                    ...
                    <div className="lg:col-span-12 xl:col-span-8 flex flex-col gap-10Order">
                        <form onSubmit={submitHandler} className="space-y-6 md:space-y-10">

                            {/* Step 1: Shipping */}
                            <div className="bg-white rounded-[2rem] md:rounded-[2.5rem] p-5 md:p-10 shadow-xl border border-purevit-primary/5">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-8">
                                    <div className="md:col-span-2 space-y-2">
                                        ...
                                        <input
                                            ...
                                            className="w-full px-4 md:px-5 py-3 md:py-4 bg-purevit-secondary/30 border border-purevit-primary/5 rounded-2xl focus:bg-white focus:border-purevit-primary transition-all outline-none font-medium text-purevit-dark text-sm"
                                            required
                                        />
                                    </div>
                            ...
                            {/* Step 2: Payment */}
                            <div className="bg-white rounded-[2rem] md:rounded-[2.5rem] p-5 md:p-10 shadow-xl border border-purevit-primary/5 relative overflow-hidden">
                                ...
                                <div className="flex items-center gap-4 md:gap-6 p-5 md:p-8 border-2 border-purevit-primary bg-purevit-primary/5 rounded-[2rem] shadow-sm relative group transition-all duration-300 active:scale-[0.99]">
                                    ...
                                    <div className="flex-grow">
                                        <span className="block text-base md:text-lg font-bold text-purevit-dark">Cash on Delivery (COD)</span>
                            ...
                            <div className="bg-white rounded-[2rem] md:rounded-[2.5rem] p-5 md:p-10 shadow-xl border border-purevit-primary/5">
                                <h2 className="text-xl md:text-2xl font-serif text-purevit-dark mb-3 md:mb-4">Delivery Notes <span className="text-xs font-sans font-bold text-gray-300 uppercase ml-2 tracking-widest">(Optional)</span></h2>
                                <textarea
                                    rows="3"
                                    placeholder="Any special instructions for the carrier?"
                                    value={userNotes}
                                    onChange={(e) => setUserNotes(e.target.value)}
                                    className="w-full px-4 md:px-6 py-3 md:py-5 bg-purevit-secondary/30 border border-purevit-primary/5 rounded-2xl focus:bg-white focus:border-purevit-primary transition-all outline-none font-medium text-purevit-dark text-sm mb-6 md:mb-10"
                                ></textarea>

                                <button
                                    type="submit"
                                    disabled={loading}
                                    className="w-full group relative py-4 md:py-7 bg-purevit-dark hover:bg-black text-white rounded-xl md:rounded-[1.5rem] font-black uppercase tracking-[0.2em] md:tracking-[0.3em] text-[10px] md:text-xs overflow-hidden transition-all duration-500 shadow-2xl active:scale-[0.98] disabled:opacity-50"
                                >
                                    <span className="relative z-10 flex items-center justify-center gap-2 md:gap-4">
                                        {loading ? (
                                            <>
                                                <span className="md:hidden">Confirming...</span>
                                                <span className="hidden md:inline">Confirming Order...</span>
                                            </>
                                        ) : (
                                            <>
                                                <span className="md:hidden">Complete · {formatPrice(subtotal)}</span>
                                                <span className="hidden md:inline">Complete Purchase · {formatPrice(subtotal)}</span>
                    ...
                    <div className="lg:col-span-12 xl:col-span-4 max-w-xl xl:max-w-none w-full mx-auto">
                        <div className="bg-white rounded-[2rem] md:rounded-[2.5rem] p-5 md:p-10 text-purevit-dark shadow-2xl sticky top-24 overflow-hidden group border border-purevit-primary/5">
                            <div className="absolute top-0 right-0 w-64 h-64 bg-purevit-primary/5 blur-[100px] rounded-full"></div>

                            <div className="flex items-center gap-3 md:gap-4 mb-5 md:mb-10 pb-4 md:pb-6 border-b border-purevit-primary/10">
                            ...
                            <div className="space-y-5 mb-6 md:mb-10 max-h-[260px] md:max-h-[300px] overflow-y-auto pr-2 scrollbar-hide">
                                {cartItems.map((item) => (
                                    <div key={item.product} className="flex gap-5 group/item">
                                        ...
                                        <div className="min-w-0 flex-grow flex flex-col justify-center">
                                            <h4 className="text-xs font-bold leading-tight whitespace-normal break-words group-hover/item:text-purevit-primary transition-colors">{item.name}</h4>
                                            <p className="text-[9px] font-black uppercase tracking-widest text-gray-400">Qty: {item.qty} · {formatPrice(item.price)}</p>
                                        </div>
                                        <div className="text-right flex flex-col justify-center">
                                            <span className="font-serif text-lg text-purevit-dark">{formatPrice(item.price * item.qty)}</span>
                                        </div>
                                    </div>
                                ))}
                            </div>
                            ...
                            <div className="space-y-4 pt-10 border-t border-purevit-primary/10">
                                <div className="flex justify-between items-center">
                                    ...
                                    <span className="font-serif text-lg text-purevit-dark">{formatPrice(subtotal)}</span>
                                </div>
                                <div className="flex justify-between items-center">
                                ...
                                <div className="flex justify-between items-end pt-4">
                                    ...
                                    <span className="text-3xl md:text-4xl font-outfit font-bold text-purevit-dark">{formatPrice(subtotal)}</span>
                                </div>
                            </div>
                        </div>
                    </div>
